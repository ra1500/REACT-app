...
class AuditQuestions extends React.Component {
  ...
  getMaxQtyAndPoints() {
        ...
        axios.get("http://localhost:8080/api/qs/q?sn=" + this.state.questionSetVersionEntityId,
  ...
  getQuestion() {
    if (this.state.currentQuestion <= this.state.questionSetSize ) {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/q/e/" + this.state.questionSetVersionEntityId + "/" + this.state.currentQuestion,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
          this.setState({
            isLoaded: true,
  ...
  getQuestion2() {
    if (this.state.jumpQuestion != null && this.state.jumpQuestion <= this.state.questionSetSize ) {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/q/e/" + this.state.questionSetVersionEntityId + "/" + this.state.jumpQuestion,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
          this.setState({
            isLoaded: true,
  ...
  getUserScore() {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/a/z?sv=" + this.state.questionSetVersionEntityId + "&fId=" + this.props.friendId,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
          this.setState({
            isLoaded: true,
    ...
    getUserAnswer() {
       ...
       const hash = btoa(token);
       const Basic = 'Basic ' + hash;
       axios.get("http://localhost:8080/api/a/b/" + this.state.questionsEntityId + "/" + this.props.friendId,
       {headers : { 'Authorization' : Basic }})
       .then((response) => {
  ...
  getQuestionSetVersionEntity() {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/qs/g?qsid=" + this.props.questionSetVersionEntityId,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
          this.setState({
            isLoaded: true,
  ...
  sendAuditToAuditee() {
        ...
        axios.post("http://localhost:8080/api/prm/sc/q?qsId=" + this.state.questionSetVersionEntityId,
        data,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
   ...
   deleteAllAnswers() {
    ...
    axios.post("http://localhost:8080/api/a/dll/" + this.state.questionSetVersionEntityId,
    data,
    {headers : { 'Authorization' : Basic }})
    .then((response) => {
