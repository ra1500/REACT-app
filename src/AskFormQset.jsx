...
class AskFormQset extends React.Component {
  ...
  jumpToSequenceNumber(value) {
   ...
   postNewQset() {
        ...
        axios.post("http://localhost:8080/api/qs/p?qsid=" + this.state.questionSetVersion, data,
    ...
    deleteAllQuestions() {
        ...
        axios.post("http://localhost:8080/api/qs/da", data,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
    ...
    inviteToScoreIndividual() {
        ...
        axios.post("http://localhost:8080/api/prm/sc/o?qsId=" + this.state.questionSetVersion, data,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
    ...
    editThisSet(value) {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/q/f/" + value,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
          this.setState({
            isLoaded: true,
    ...
    manageAset(event) {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/q/f/" + event.target.value,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
          this.setState({
            isLoaded: true,
  ...
  render() {
    ...
    <React.Fragment>
      ...
      <form onSubmit={this.handleSubmit1}>
          ...
          <div class="askDiv"><span class="askText">Description &nbsp;</span> <input id="askForm2" type="text" maxlength="49" value={this.state.description} onChange={this.handleChange3} /></div>
          <br></br>
